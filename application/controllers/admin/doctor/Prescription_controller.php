...
#-----------------------------------------	 
	public function save_prescription()
	{
		...
		// Check if $medd is empty and set it to a default value if necessary
		if (empty($medd['medicine'])) {
			$medd = array('medicine' => array());
		}
		...
		$data['heartrate'] = $this->input->post('heartrate');
		$data['resprate'] = $this->input->post('resprate');
...
#-------------------------------------------		 
	...
	public function get_prescription($prescription_id)
	{
		...
		// Fetch patient data
		$pres_data['patient'] = $this->db->select('*')
			->from('patient_tbl')
			->where('patient_id', $this->input->post('patient_id'))
			->get()
			->row();

		// Fetch print pattern data
		$pres_data['pattern'] = $this->db->select('*')
			->from('print_pattern')
			->get()
			->row();

		// Load the appropriate view based on pattern
